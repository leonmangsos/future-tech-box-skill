...
#### v0.2.1-beta (2026-08-07)
- 🛡️ **烧录策略优化：防止 USB 接口电流过载**
  - 烧录命令从 `pio run -t upload`（会 hard_reset 自动运行程序）改为 **esptool 直调 + `--after no_reset`**
  - 烧录完成后程序**不会自动运行**，芯片停留在下载模式
  - 用户需**重新开关主板电源**后程序才会运行，避免烧录瞬间驱动电机触发笔记本 USB 过载提醒
  - 同步更新：烧录成功提示、串口监视器说明、故障排查指南、`upload_with_retry.py` 脚本
