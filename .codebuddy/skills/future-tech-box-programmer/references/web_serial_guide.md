...
### 2.0 专用说明（XIAO ESP32S3）
- 串口走 **ESP32-S3 内置 USB-Serial/JTAG**（VID:PID `303A:1001`）
- **无需安装任何驱动**，Windows/macOS/Linux 原生识别为 COM 口
...
### 3.3 场景 3：PS2 手柄 → 串口转发（控制网页游戏/动画）
...
| 连接成功但无数据 | 波特率不一致 | 固件和网页都改成 115200 |
