...
# 部分批次可能使用 ESP32-S3 内置 USB-Serial/JTAG（303A:1001），兼容匹配
...
def pick_candidates(devices: List[Dict[str, str]]) -> List[Dict[str, str]]:
    ...
    for d in devices:
        ...
        vid = d.get("vid", "")
        pid = d.get("pid", "")
        # CH343 (1A86:55D3) 或 ESP32-S3 内置 USB (303A:1001)
        if vid in TARGET_VIDS and pid in TARGET_PIDS:
            d["chip_type"] = "CH343" if vid == "1A86" else "ESP32-S3 native USB"
            result.append(d)
    return result
    ...
    try:
        ...
        if len(candidates) == 1:
            output(
                {
                    "status": "ok",
                    "message": f"已识别到未来科技盒 3.0 串口（{candidates[0].get('chip_type', '')}）",
                    "result": candidates[0],
                }
            )
            return
        ...
        if len(candidates) == 0:
            output(
                {
                    "status": "not_found",
                    "message": "未检测到 CH343(1A86:55D3) 或 ESP32-S3(303A:1001) 已连接端口",
                    "result": {
                        "port": "",
                        "vid": "",
                        "pid": "",
